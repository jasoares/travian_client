...
fake_building_gids_lookup_page

module Travian
  module Helpers
    describe BuildingHelper do
      describe '.building_gids' do
        it 'returns a hash with symbol keys to each building and corresponding gids' do
          BuildingHelper.building_gids.should be_a Hash
        end

        it 'should have keys of type Symbol only' do
          BuildingHelper.building_gids.keys.all? {|k| k.is_a? Symbol }
        end

        it 'should have values of type String only' do
          BuildingHelper.building_gids.values.all? {|v| v.is_a? String }
        end

        it 'should return the "21" when asked for the key :workshop' do
          BuildingHelper.building_gids[:workshop].should == '21'
        end
        ...
        context 'given the data as of September 19 2012' do
          ...
          it 'should match the same gid\'s' do
            BuildingHelper.building_gids.should == @bgids
          end
        end  
      end
